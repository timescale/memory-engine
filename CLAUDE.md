...
- **API**: JSON-RPC 2.0 over HTTP, two endpoints:
  ...
  - Plus REST OAuth device-flow endpoints under `/api/v1/auth/*`, browser login/logout (`/api/v1/auth/login/:provider`, `/api/v1/auth/logout`), and the **web UI served at root `/`** (any non-`/api` GET → static assets / SPA fallback).
...
- **Two endpoints, two auth modes**: memory RPC (session or api key + `X-Me-Space`) vs user RPC (session only). `extractSessionCredential` resolves the credential (`Authorization` Bearer — session or api key — else the `me_session` cookie).

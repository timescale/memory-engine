...
- **Harness surfaces (`me mcp`, capture hooks, harness shells)**: a plain `me` invocation always runs as the principal represented by the presented credential (session, OAuth token, user PAT, or service-account key). There is no impersonation, no `--as-agent`, no `ME_AS_AGENT`, and no `X-Me-As-Agent`. The one thing a harness still injects into every shell command is the profile-discovery anchor `ME_PROJECT_DIR` — set by Claude's SessionStart hook (`me claude env` → `$CLAUDE_ENV_FILE`), opencode's `shell.env` plugin hook, and Codex's command-string rewrite (`me codex env-hook`, prepending an `export …; ` prefix via `renderExportPrefix` — see `packages/cli/codex/env-hook.ts`; it fails open and logs an unrecognized payload's STRUCTURE, never its content, via `packages/cli/harness-shape-log.ts`, for a later `me doctor` to surface). The contract lives in `packages/cli/harness-contract.ts` and consists of two vars: `AI_AGENT` (inert identity metadata, per the `@vercel/detect-agent` convention) and `ME_PROJECT_DIR` (the anchor `me` resolves machine-local harness policy from — see the CLI-credentials bullet below; it does NOT trigger any repository-config discovery). Legacy compat: `X-Me-As-Agent` is silently ignored server-side.
- **Embedding**: Vercel AI SDK; OpenAI `text-embedding-3-small` (1536-dim) in production; Ollama supported for local dev.
- **CLI**: `me` binary — `login`, `logout`, `whoami`, `space`, `group`, `access`, `service`, `apikey`, `memory` (+ top-level aliases like `me search`, `me create` — except `import`), `import` (the source group: `memories`/`claude`/`codex`/`opencode`/`granola`/`git`/`docs`/`slab`; `me memory import` and `me <tool> import` remain as aliases), `mcp`, `init`/`doctor` (machine-local harness policy), `claude`/`codex`/`opencode`, `serve`, `pack`.
...
(`claude` and `opencode` verified live) non-interactively and check that
...
- **CLI credentials**: split across `~/.config/me/` — **`config.yaml`** (non-secret: `default_server` + per-server **active space** / the X-Me-Space) and **`credentials.yaml`** (0600, secret session-token *fallback* only). The **session token** lives in the OS keychain when available (macOS `security`, Linux `secret-tool` via libsecret; `ME_NO_KEYCHAIN=1` forces off), else in `credentials.yaml` (empty/absent on keychain hosts); a pre-split `credentials.yaml` is migrated on first read. `me logout` clears the session secret but keeps the non-secret config (so re-login resumes). **Api keys are never persisted** — user PATs and service-account keys only ever come from `ME_API_KEY`/`--api-key`; `apiKey.create` prints the key once for the operator to place where it runs. Env: `ME_SERVER` / `ME_API_KEY` / `ME_SPACE` / `ME_SESSION_TOKEN` / `ME_NO_KEYCHAIN` / `ME_PROJECT_DIR`. Resolution is **machine-local only**: `resolveServer`/`resolveSpace`/`resolveCredentials` (in `packages/cli/credentials.ts`) read flags, `ME_*` env, and the global `config.yaml`, with precedence `--flag > ME_* env > global config.yaml (default_server / per-server active_space) > default`. There is **no repository config** — no `.me/config.yaml`, no walk-up discovery, no `--config-dir`/`ME_CONFIG_DIR`, no server whitelist, no `agent:` field. **Machine-local harness policy** (which harness gets MCP tools, session capture, or CLI server/space targeting) lives in the SAME `~/.config/me/config.yaml` under `version` / `defaults` / `directories.<canonical-abs-path>` — each a profile with `mcp` / `capture` / `cli` surfaces — written by `me init`, pruned per-harness on uninstall, and resolved in `packages/cli/local-config.ts` by longest-ancestor directory match anchored at `ME_PROJECT_DIR ?? cwd`. It is consumed only on the harness surfaces — `me mcp` (mcp surface), the capture hooks (capture surface), and harness-shell CLI routing (cli surface, seeded in `index.ts`'s `preAction` only when `AI_AGENT` names a known harness) — never in ordinary human CLI resolution, and is inspected with `me doctor`. `ME_PROJECT_DIR` is that profile-discovery anchor (set by the harness adapters into every shell command), not a repository-config trigger.

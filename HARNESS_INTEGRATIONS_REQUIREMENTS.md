...
### 2.4 `me init [directory] [--global] [flags]`
...
- Non-interactive flags:
  ...
  - `--capture-tree-root <path>` (global scope only)
  ...
  - `--capture-tree` requires directory scope; `--capture-tree-root` requires
    global scope; mixing them is an error.
...
- `me project init` — replaced by `me init`. Retained one release as a hard
  redirect that prints a deprecation warning naming the new command.
- `me project ci` — replaced by `me ci install`. Same hard-redirect treatment.
...
defaults:
  ...
  capture:
    ...
    server: https://api.memory.build
    space: <slug>
    tree_root: ~/projects        # global capture uses tree_root
    harnesses:
      claude: false
      opencode: false
      codex: false
...
- **`capture`** — controls transcript collection.
  ...
  - Destination:
    ...
    - Global (`defaults`) profile uses `tree_root` (slug-free parent; the
...
1. **Scope**
   - Skipped when `me init <dir>` or `me init --global` is passed.
...
6. **Capture surface**
   ...
   - Global scope: prompt for `tree_root`; also print an explicit privacy note
...
- A surface `enabled: true` with no harnesses selected: refuse; loop back on
  the harness step.
- Directory scope + `tree_root`, or global scope + `tree`: refuse.
...
- The runtime, capture hooks, MCP dispatcher, `me init`, and CLI commands
  **do not read `.me/config.yaml`**.
- Existing pins in a repo have no effect on behavior.
- One migration surface exists: `me init <dir>` may detect a `.me/config.yaml`
  in the target directory and, with explicit user confirmation, import its
  `server`, `space`, `tree`, and `capture` values into the directory profile
  it is about to write. This is opt-in per-run, user-attended, and does not
  cause a cloned repo to affect anyone silently.
- After the two-release migration window, `me init` stops offering the import.

---

## 8. Deprecations

Two-release window. Approximate release plan (specific numbers TBD by
release planning):

- **Release N**
  - `me project init` and `me project ci` remain as hard redirects that print
    a warning and exit non-zero, naming the new commands. No behavioral shim.
  - `.me/config.yaml` is no longer consulted at runtime. `me init` offers a
    one-time migration import when it finds one.
- **Release N+1**
  - Deprecation warnings from the redirects grow more prominent, naming the
    removal release.
- **Release N+2**
  - `me project init` and `me project ci` removed.
  - `me init`'s legacy `.me/config.yaml` migration import removed.
...
- Deprecated `me project init` / `me project ci` invocations print the
  deprecation warning and exit non-zero.

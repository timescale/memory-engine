...
- `ME_SPACE` or `--space` \- the slug of a space to scope commands to
...
You must be a space admin or be a member of the group with the `admin` flag on the membership in order to remove members. Member must be in the `core.principal_space` table for this space.

...
create unique index on {{schema}}.principal (name) include (user_id) where user_id is not null;
-- each user's agents must have a unique name (per that user)
create unique index on {{schema}}.principal (owner_id, name) where agent_id is not null;
-- each space's groups must have a unique name (per that space)
create unique index on {{schema}}.principal (space_id, name) where group_id is not null;

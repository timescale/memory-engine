 ...
 * model: principals (users/groups/service accounts), space membership, group membership,
 * and 3-level tree-access grants. All methods are scoped to the
 * space selected by the X-Me-Space header and require space-owner authority.
 *
 * "Principal" is the union (user | group | service account); the space
 * roster holds principals. "Member" is reserved for credential-bearing
 * principals (users/service accounts): group members and api-key holders.
 */
import { z } from "zod";
import { nameSchema, uuidv7Schema } from "../fields.ts";

/** Principal kind: user / group / service account. */
export const principalKindSchema = z.enum(["u", "g", "s"]);
export type PrincipalKind = z.infer<typeof principalKindSchema>;

/**
 * A principal on a space's roster — i.e. with a direct membership row
 * (principal_space). Users, groups (a group is rostered into its
 ...
 * admin group; false for a group rostered admin=false).
 */
...
// shared by group.* mutation results
export { nameSchema };

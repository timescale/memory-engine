...
test("an agent caller is denied every account-management method (FORBIDDEN)", async () => {
  ...
  const denied: [string, unknown][] = [
    ...
    ["serviceAccount.create", { spaceId: agentId, name: "x" }],
    ["serviceAccount.list", { spaceId: agentId }],
    ["serviceAccount.rename", { id: agentId, name: "x" }],
    ["serviceAccount.delete", { id: agentId }],

 ...
 *     `home.<member>` for a user. Ids have hyphens stripped to valid ltree
 *     labels. `~` → the prefix, `~/bar` →
 ...
 * valid ltree labels). Mirrors core `add_principal_to_space`.
 */
export function homePrefix(principalId: string): string {
  return `${HOME_NAMESPACE}.${homeLabel(principalId)}`;
}
...
export function normalizeTreePath(
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (seg === "~") {
      ...
      out.push(homePrefix(opts.home)); // valid `home.<…>` ltree
      continue;
    }
...
export function normalizeTreeFilter(
  ...
  if (s === "~" || s.startsWith("~/") || s.startsWith("~.")) {
    ...
    s = homePrefix(opts.home) + s.slice(1); // "~/foo" → "<prefix>/foo"
  }

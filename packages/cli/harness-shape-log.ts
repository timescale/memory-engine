 ...
 * Codex's rewrite hook is fail-open on anything it doesn't
 * understand — an internal error or a payload that doesn't match the
 * vendored shape. A silent fail-open would otherwise leave "the project
 * discovery anchor isn't landing in shell tools" with no trail back to
 * "Codex shipped a payload update". So every unrecognized-shape
 * fail-open also appends one line here — STRUCTURE only (top-level keys,
 * sorted; never values, since a command string can carry secrets) — for a
 * future diagnostic command to summarize as "N unrecognized payload shapes
 * since <date>: upgrade `me` or file an issue."
 *

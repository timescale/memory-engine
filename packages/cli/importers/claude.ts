...
async function parseSessionFile(file: string): Promise<ImportedSession | null> {
  ...
  for (const line of lines) {
    ...
    if (typeof event.uuid === "string") lastMessageId = event.uuid;
    ...
    for (const block of blocks) {
      if (type === "user" && block.kind === "text") {
        ...
        turns.push({ role: "user", text: unwrapped, timestamp });
        counts.user++;
      } else if (type === "user" && block.kind === "tool_result") {
        turns.push({
          role: "tool_result",
          text: block.text,
      ...
      } else if (type === "assistant" && block.kind === "text") {
        turns.push({ role: "assistant", text: block.text, timestamp });
        counts.assistant++;
      } else if (type === "assistant" && block.kind === "thinking") {
        turns.push({ role: "reasoning", text: block.text, timestamp });
      } else if (type === "assistant" && block.kind === "tool_use") {
        turns.push({
          role: "tool_call",
          text: block.text,
          toolName: block.toolName,
...
/** Normalized content block extracted from a Claude message. */
...
/**
 * Normalize Claude `message.content` (string or block array) into a list
 * of normalized text blocks.
 */

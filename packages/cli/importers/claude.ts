...
/** Claude-generated XML wrapper tags that are not real user prompts. */
const GENERATED_USER_TAGS = [
  "system-reminder",
  "task-notification",
  "ultrawork-mode",
  "ide_opened_file",
...
function extractBlocksForEvent(
  ...
  for (const block of raw) {
    if (type === "user" && block.kind === "text") {
      const sanitized = sanitizeUserText(block.text);
      if (sanitized === null) continue;
      out.push({ kind: "text", text: sanitized });
    } else {
      out.push(block);
    }
  }
...
/**
 * Strip Claude-generated wrapper text from a user block and return the real
 * prompt, or null when the block is pure metadata / replay noise.
 */
export function sanitizeUserText(text: string): string | null {
  const original = text.trim();
  const unwrapped = unwrapSdkReplayBundle(text);
  if (unwrapped === null) return null;

  const stripped = stripGeneratedUserText(unwrapped);
  if (stripped === null) return null;

  const trimmed = stripped.trim();
  if (trimmed.length === 0) return null;
  if (isLocalCommandWrapper(trimmed)) return null;
  if (isGeneratedUserNotification(trimmed)) return null;
  if (original.startsWith("Human:") && isLikelyStandaloneToolOutput(trimmed)) {
    return null;
  }
  return trimmed;
}
...
/**
 * Remove leading/suffix Claude-generated notification blocks while preserving
 * any real prompt text before or after them.
 */
function stripGeneratedUserText(text: string): string | null {
  let current = stripTrailingGeneratedBlock(text).trim();

  while (current.length > 0) {
    const next = stripLeadingGeneratedBlock(current).trim();
    if (next === current) break;
    current = stripTrailingGeneratedBlock(next).trim();
  }
...
/**
 * Drop generated suffixes like a plan/build reminder appended after a real
 * prompt on the same user message.
 */
function stripTrailingGeneratedBlock(text: string): string {
  const match = text.match(
    /\n\s*(?:<!-- OMO_INTERNAL_INITIATOR -->\s*)*<(system-reminder|task-notification|ultrawork-mode|ide_opened_file)>[\s\S]*$/,
  );
...
/**
 * Drop generated leading blocks that sometimes precede the real prompt.
 */
function stripLeadingGeneratedBlock(text: string): string {
  let current = text.trimStart();

  for (const tag of GENERATED_USER_TAGS) {
    if (!current.startsWith(`<${tag}>`)) continue;
    const endTag = `</${tag}>`;
    const endIdx = current.indexOf(endTag);
    if (endIdx < 0) return "";
    current = current.slice(endIdx + endTag.length).trimStart();
    ...
    current = current.replace(/^<!-- OMO_INTERNAL_INITIATOR -->\s*/, "");
...
/**
 * True for bracketed/request-notification payloads that are generated by the
 * Claude CLI / SDK rather than authored by the user.
 */
function isGeneratedUserNotification(text: string): boolean {
  return (
    text.startsWith("[Tool Result for ") ||
    text.startsWith("[Request interrupted by user") ||
    text.startsWith("[Assistant: ") ||
    text.startsWith("[Tool Use:") ||
    text.startsWith("<task-notification>") ||
    text.startsWith("<system-reminder>") ||
    text.startsWith("<ultrawork-mode>") ||
    text.startsWith("<ide_opened_file>") ||
    text.startsWith("<!-- OMO_INTERNAL_INITIATOR -->") ||
    /^Read the output file to retrieve the result:\s*\S+$/.test(text)
  );
}

/**
 * Heuristic for sdk-ts `Human: ...` wrapper payloads that are plainly tool or
 * progress output rather than an authored user prompt.
 */
function isLikelyStandaloneToolOutput(text: string): boolean {
  return (
    /^(?:[│◆●]|Error: |error: |On branch |diff --git |go: downloading |package [a-z_])/m.test(
      text,
    ) ||
    /Skipped \d+ suggested fixes\./.test(text) ||
    /Checked \d+ files in \d+ms\./.test(text) ||
    /Found \d+ errors?\./.test(text) ||
    /script ".+" exited with code \d+/.test(text) ||
    /Run '.+ --help' for usage\./.test(text) ||
    /Command exited with code \d+/.test(text)
  );
}

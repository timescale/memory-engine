...
export type PrincipalKind = "u" | "g" | "s";
...
export interface ValidatedApiKey {
  /** The principal (user or service account) the key belongs to. */
  memberId: string;
  /** The api_key row id. */
  apiKeyId: string;
  /**
   * The member's kind — always one of u|s in practice (groups hold no key).
 ...
 * Users, groups (a group is rostered into its space on creation), and
...
/** A member (user / service account) of a group, with the group admin flag. */

...
- [x] **Post-ship — tree rewrite: full hierarchy via `memory.tree`, lazy leaves.** The original implementation fed `memory.search` results into the tree and was capped at 1000 rows, producing a confusing truncated view. The tree now fetches the full path hierarchy from `memory.tree` (all prefixes + aggregate counts, no content) and renders every path regardless of memory volume. Memory leaves are loaded lazily per expanded path via `memory.search` with an lquery pattern that forces exact-path match: `last|last` for non-empty paths, `*{0,0}` for the empty-tree bucket. `exactTreeLquery(path)` encapsulates the construction. `PathRow` gates the fetch on `expanded && directCount > 0` so container paths with no direct memories skip the round-trip. `directCount = aggregateCount - Σ children.aggregateCount` is derived client-side from the tree RPC output, so the UI knows whether a path has any inline leaves without a probe. The synthetic `.` node is driven by an always-on `memory.search({tree: "*{0,0}"})` query — it appears only when `total > 0` and reuses the cached results when the node is expanded. Mutation invalidations widened to cover both new query keys (`memory-tree`, `memories-at-exact-path`). Search UI (SimpleSearchBar/AdvancedSearchPanel) is temporarily unmounted; the stores/hooks remain intact and will be rewired to overlay on the full tree in a follow-up. 27 web-package tests pass (12 for `buildPathTree`/`collectPaths`, 3 for leaf helpers, 5 URL state, 9 frontmatter). Full check clean: 674 tests, 0 failures.
- [x] **Post-ship — search UX restored as a tree-building dual mode.** `TreeView` now runs in one of two modes driven by `selectSearchParams(filter)`: when the returned params are empty (no active criterion), browse mode renders the full hierarchy from `memory.tree` with lazy leaves (prior behavior); when any criterion is set, search mode runs `memory.search` and feeds the results into a new `buildSearchTree(memories)` that produces a tree with inline leaves — no lazy fetch per path. `PathNode` gained an optional `inlineLeaves?: MemoryLeaf[]` field; `PathRow` prefers it when present and also accepts a `forceOpen` prop so search-mode paths render expanded by default (user can still collapse them after). The browse-mode and search-mode queries are mutually gated by `enabled: searchActive`/`!searchActive` so only one set hits the wire at a time. `useMemories(params, enabled?)` gained the optional enabled flag. `app.tsx` remounts `SimpleSearchBar` and (on advanced mode) `AdvancedSearchPanel`; the filter store + URL round-trip were already intact from the prior pass. 32 web tests pass (5 new `buildSearchTree` cases covering grouping, empty-tree bucket, temporal-desc inline leaves, ancestor aggregate counts). Full check clean: 679 tests, 0 failures.

...
The active space is saved as the per-server `active_space` in your global
`~/.config/me/config.yaml`. It applies to the resolved server (the `--server`
flag or `ME_SERVER`, else your default server).

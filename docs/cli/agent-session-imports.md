...
**Every session routes by its own project's config**, exactly like the live capture hook: each session's recorded cwd is resolved against that project's committed [`.me/config.yaml`](../project-config.md) — its **server** (whitelist-gated, the same credential-safety rule as a local run), **space**, and **tree** (sessions nest directly under it, `<tree>/<sessions-node-name>/…`, no slug). Sessions from projects without a `.me` use the run's own server/space and the `~/projects` + per-slug layout. The documented precedence holds throughout — an explicit `--server`/`--tree-root` flag or `ME_*` env still outranks any project's `.me`. One exception: `--config-dir`/`ME_CONFIG_DIR` pins a *single* project's config, which contradicts per-session routing, so a sweep rejects it with an error — scope with `--project <path>`, or force a target with `ME_SERVER`/`ME_SPACE`/`--tree-root` instead. (Single-target commands — `me import git`, the capture hooks, `me mcp` — keep honoring the pin.)

A project the sweep can't safely write for is **skipped and tallied** (reported as session skip reasons) instead of failing the run: `project_config_error` (resolving the project threw — a malformed `.me`, a server outside your [trusted list](../project-config.md#trusted-servers-credential-safety), a `.me` agent sentinel with no project agent, …; `--verbose` prints the underlying message per session), `no_credentials_for_server` (you're not logged in there and hold no api key), or `no_space_for_project`. Run a local import inside that project to reproduce the error loudly.

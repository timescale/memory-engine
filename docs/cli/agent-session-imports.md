...
| `--tree-root <path>` | Tree root under which `<slug>/<sessions-node-name>` nodes are placed. Default: the private `~/projects` (your own home). Accepts ltree labels (`[A-Za-z0-9_-]`) separated by `/`, with an optional leading `~` for your home. |

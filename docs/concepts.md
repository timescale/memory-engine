...
- **`/home/<member_id>`** -- your private user root. The input shortcut **`~`** expands to your own home, so `~/notes` resolves to `/home/<your-id>/notes` and displays back as `~/notes`. **Service accounts** do not get a home grant; use `/share/...` or another explicitly granted tree for their memories.

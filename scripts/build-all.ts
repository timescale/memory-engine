...
/** Bun leaves these compiler intermediates behind on some platforms. */
async function cleanupBunBuildArtifacts() {
  const files = await readdir(".");
  await Promise.all(
    files
      .filter((file) => file.startsWith(".") && file.endsWith(".bun-build"))
...
<dict>
    ...
    for (const { suffix } of targets.filter((t) =>
      t.target.includes("darwin"),
    )) {
